...
from transwarp.web import get, post, view, ctx, view, interceptor, seeother, notfound
...
def register_user():
    ...
    user = User(name=name, email=email, password=password, image='http://www.gravatar.com/avatar/%s?d=mm&s=120'
...
def manage_blogs_edit(blog_id):
    ...
    return dict(id = blog.id, name = blog.name, summary = blog.summary, content = blog.content,
                action = '/api/blogs/%s' % blog_id, redirect = '/manage_blog', user = ctx.request.user)
